...
|-- database/                         # Skema SQL, seed data, dan collection API
|   |-- schema.sql                    # Skema tabel MySQL untuk semua service
|   |-- seed.sql                      # Data awal zona, warga, sensor, cuaca, dan laporan
|   `-- smartcity.postman_collection.json
|                                      # File collection Postman untuk testing endpoint
|-- gateway/                          # API Gateway Express.js
|   |-- src/
|   |   `-- gateway.js                # Routing, middleware JWT, logging, rate limiting
|   |-- Dockerfile                    # Image gateway
|   |-- package.json                  # Dependency Node.js gateway
|   `-- .env.example                  # Contoh environment gateway
|-- services/
|   `-- auth/                         # OAuth Server / Auth Service
|       |-- src/
|       |   |-- app.js                # Aplikasi Express OAuth
|       |   |-- config/               # Konfigurasi database auth
|       |   `-- middleware/           # JWT middleware
|       |-- db_auth.sql               # Tabel tambahan auth jika dipisah
|       |-- Dockerfile                # Image auth service
|       `-- package.json              # Dependency OAuth server
|-- php-citizen/                      # Citizen Service PHP MVC
|   |-- app/
|   |   |-- Controllers/              # CitizenController, ReportController, NotifController
|   |   |-- Models/                   # Citizen, Report, Notification
|   |   |-- Validators/               # Validasi input citizen/report/notif
|   |   `-- Config/                   # Koneksi database
|   |-- public/index.php              # Router Citizen Service
|   |-- Dockerfile                    # Image Citizen Service
|   `-- composer.json                 # Dependency PHP Citizen Service
|-- air-quality-service/              # Air Quality Service PHP MVC
|   |-- app/
|   |   |-- Controllers/              # AirQualityController, StationController, ReadingController
|   |   |-- Models/                   # AirReading, AirStation
|   |   |-- Services/                 # AQI calculator dan RabbitMQ publisher
|   |   |-- Validators/               # Validasi payload sensor udara
|   |   `-- Config/Database.php       # Koneksi PDO ke MySQL
|   |-- public/index.php              # Router Air Quality Service
|   |-- Dockerfile                    # Image Air Quality Service
|   `-- composer.json                 # Dependency PHP Air Quality Service
|-- environment-service/              # Environment Service PHP MVC
|   |-- app/
|   |   |-- Controllers/              # WeatherController, AlertController, ZoneController
|   |   |-- Models/                   # Weather, Alert, Zone
|   |   |-- Config/                   # Konfigurasi database dan RabbitMQ
|   |   `-- Services/                 # Publisher/consumer event environment
|   |-- consumer.php                  # RabbitMQ consumer anomaly.alert
|   |-- public/index.php              # Router Environment Service
|   |-- Dockerfile                    # Image Environment Service
|   `-- composer.json                 # Dependency PHP Environment Service
|-- ml-service/                       # Python ML Service dan IoT simulator
|   |-- app.py                        # FastAPI endpoint prediksi dan anomaly detection
|   |-- simulator.py                  # Simulator IoT publish data ke MQTT
|   |-- generate_dataset.py           # Generator dataset sintetis
|   |-- train_models.py               # Training model ML
|   |-- models/                       # File model hasil training
|   |-- data/                         # Dataset CSV
|   |-- Dockerfile                    # Image Python ML Service
|   `-- requirements.txt              # Dependency Python
|-- iot/                              # Konfigurasi IoT dan Node-RED
|   |-- mosquitto.conf                # Konfigurasi MQTT broker Mosquitto
|   `-- node-red-data/
|       `-- flows.json                # Flow Node-RED airquality dan weather bridge
|-- k8s/                              # Manifest deploy ke Kubernetes
|   |-- namespace.yaml                # Namespace smartcity
|   |-- configmap.yaml                # Variabel non-sensitif
|   |-- secrets.yaml                  # Kredensial service
|   |-- mysql-statefulset.yaml        # StatefulSet dan PVC MySQL
|   |-- rabbitmq-deployment.yaml      # Deployment dan Service RabbitMQ
|   |-- gateway-deployment.yaml       # Deployment dan Service API Gateway
|   |-- php-deployments.yaml          # Deployment Citizen, AirQuality, Environment
|   |-- python-ml-deployment.yaml     # Deployment dan Service ML
|   |-- ingress.yaml                  # Routing eksternal
|   `-- hpa.yaml                      # Horizontal Pod Autoscaler
|-- monitoring/                       # Monitoring Prometheus dan Grafana
|   |-- prometheus.yml                # Scrape config semua service
|   `-- grafana-dashboard.json        # Export dashboard Grafana
|-- .github/
|   `-- workflows/
|       `-- main.yml                  # CI/CD build dan push Docker image
|-- docker-compose.yml                # Orkestrasi utama semua container
|-- docker-compose.dev.yml            # Override untuk mode development
|-- .env.example                      # Template environment seluruh sistem
